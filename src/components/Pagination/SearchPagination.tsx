...
import { setFoundPostsCurrentPage } from "../../redux/action-creators";
import { LeftArrowIcon } from "../Icons/LeftArrowIcon";
import { THEMES } from "../../constants/theme-constants";
import { SelectCountOfFoundPosts } from "../SelectCountOfPosts";
...
const SearchPagination = () => {
  ...
  const countOfPosts = useSelector(
    (state: IStoreState) => state.posts.countOfFoundPosts
  );
  ...
  const handlerSetCurrentPage = (currentPage: number) => {
    dispatch(setFoundPostsCurrentPage(currentPage));
    if (currentPage === 1) {
  ...
  const handleSwitchNextPage = () => {
    dispatch(setFoundPostsCurrentPage(currentPage + 1));
